...
def train(args, model, device, train_loader, optimizer, epoch, MILoss):
    model.train()
...
def test(model, device, test_loader, MILoss):
    model.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        train(args, model, device, train_loader, optimizer, epoch, MILoss)
        test(model, device, test_loader, MILoss)
        scheduler.step()
